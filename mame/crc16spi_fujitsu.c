...
uint16_t crc16spi_fujitsu_word(uint16_t crc, void const* mem, size_t len)
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    crc = table_word[7][word & 0xff] ^ table_word[6][(word >> 8) & 0xff] ^ table_word[5][(word >> 16) & 0xff] ^ table_word[4][(word >> 24) & 0xff] ^ table_word[3][(word >> 32) & 0xff] ^ table_word[2][(word >> 40) & 0xff] ^ table_word[1][(word >> 48) & 0xff] ^ table_word[0][word >> 56];
  }
  data += n << 3;
  len &= 7;
  crc = swaplow(crc);
...
uint16_t crc16spi_fujitsu_comb(uint16_t crc1, uint16_t crc2,
    uintmax_t len2)
{
  crc1 ^= 0x1d0f;
